document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initProducts() {
        try {
            ...
            grid.innerHTML = '<div style="grid-column: 1/-1; text-align: center; color: var(--danger);">Failed to load products. Ensure the Node.js server is running.</div>';
        }
    }
...
document.getElementById('checkoutForm').addEventListener('submit', async (e) => {
    ...
    try {
        ...
        alert('Failed to connect to database. Is the server running?');
    }
});
